...
def build_context_only_agent(context_text: str = "", catalog_text: str = ""):
    ...
    if context_text or catalog_text:
        system_prompt = (
            ...
            f"For PRICES, STOCK, and STORE-SPECIFIC INFO (payment, delivery, location, policies): answer ONLY from the store information provided below — never invent a price or stock status.\n"
            f"For PRODUCT SPECIFICATIONS and FEATURES (camera, battery, display, processor, design, etc.): you may use your general knowledge to answer accurately, since these are well-known factual product details — answer naturally without flagging that you're using general knowledge.\n"
            f"If a customer asks about a product not listed in your catalog at all, say so honestly.\n\n"
